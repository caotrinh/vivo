...
public class FreemarkerHttpServlet extends VitroHttpServlet  {
    ...
    // SCHOLDEV-469: Only called if the 404 error page is being rendered.
    // Without this call, the resulting DB connection is never closed.
    public static void closeModel(HttpServletRequest request) {
        if (ModelAccess.isPresent(request)) {
            ModelAccess.on(request).close();
        }
    }
