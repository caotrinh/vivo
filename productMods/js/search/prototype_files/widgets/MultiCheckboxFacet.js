...
AjaxSolr.MultiCheckboxFacet = AjaxSolr.AbstractFacetWidget.extend({
  ...
  constructor: function (attributes) {
      AjaxSolr.MultiCheckboxFacet.__super__.constructor.apply(this, arguments);
  ...
  afterRequest: function () {
    ...
    if (list = this.buildFacet()) {
      ...
      var heading = $('<h4></h4>').addClass('panel-heading').text(this.title).prepend('<span class="icon"></span>');
      target.append(heading).append(list).addClass('active-facet');
    }
  },

  template: function(name, count, checked, cssClass, id, handler){
    var text = (count > 0) ? name+' ('+count+')' : name;
    if (count == 0) cssClass += ' empty';
    var checkbox = $('<input/>').attr('type','checkbox').attr('name', name).addClass('checkbox').change(handler);
    var label = $('<label/>').addClass('facet-item-label');
    if(id !== undefined){
      checkbox.attr('id', id);
      label.attr('for', id);
    }
    ...
    label.append(text);
    return $('<li/>').addClass('list-group-item').addClass(cssClass).append(checkbox).append(label);
  },
  ...
  buildFacet: function() {
  	var self = this;
    var facets = this.responseFacetsMap();
    ...
    var list = $('<ul></ul>');
    list.append(self.template('Any', 0, this.isEmpty(), 'select-any', this.id+'-all', this.allResultsClickHandler()));
    ...
    if (facets.length != 0) {
      for (i=0; i<facets.length; i++) {
        ...
        if (i < limit) {
          ...
          list.append(self.template(title, facets[i].count, checked, 'unchecked', id, handler));
          count++;
        }
        else if (activeIndex > -1) { // Then append any other active checkboxes that have facet counts
          checked = true;
          activeFacets.splice(activeIndex,1);
          list.append(self.template(title, facets[i].count, checked, 'checked', id, handler));
          count++;
    ...
    if (activeFacets.length > 0) {
      for (var i=0; i < activeFacets.length; i++) {
        var id = this.id + '-' + count;
        var handler = this.clickHandler();
        list.append(self.template(activeFacets[i], 0, true, '', id, handler));
      }
    }

    if (popups[this.id] !== undefined && includeShowallLink) {
      var x = $(this.alignedTarget).offset().left + $(this.alignedTarget).outerWidth() + 30;
      var y = $(this.alignedTarget).offset().top;
      list.append($('<li class="select-more"></li>').append($('<a href="#" />').addClass('facet-showall').text('Show all...').click(this.popupHandler(x,y))));
    }
    ...
    return function() {
      ...
      if (popups[self.id] !== undefined) {
        ...
        if (self.manager.widgets[self.id+'_popup'] === undefined) {
          ...
          target.html($('<img>').attr('src', 'images/ajax-loader.gif'))
        ...
        target.dialog(widget.popupOptions(x,y));
      }
      return false;
    }
  },
