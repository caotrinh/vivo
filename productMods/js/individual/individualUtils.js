...
$(document).ready(function(){
    ...
    $('.overview-value').truncate({max_length: 1500});
    ...
    */
    ...
    $.each($('h3'), function() {
        if ( $(this).next().hasClass("subclass-property-list") && $(this).next().hasClass('hideThis')) {
            if( $(this).next('table').length > 0 && $(this).next("table").find('tbody tr').length == 0 ){
    ...
    //to hide top publications if all top publications are hidden
    $.each($('ul#toppub-noRangeClass-List').find('table.hideThis'), function(){
        if( $(this).find('tbody tr').length == 0){
            $(this).remove();
        }
    });

    // //to hide editorof if no editorof pubs
    $.each($('ul#relatedBy-Editorship-List').find('table.hideThis'), function(){
        if( $(this).find('tbody tr').length == 0){
            $(this).remove();
        }
    });
    ...
    if ( $('ul#RO_0000053-ResearcherRole-List').children('li').length < 1 &&
            ...
            $('h3#RO_0000053-InvestigatorRole').attr('class') != "hiddenGrants" ) {
