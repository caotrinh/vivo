...
class IndividualResponseBuilder {
	...
	ResponseValues assembleResponse() throws TemplateModelException {
	    ...
	    body.put("labelCount", getLabelCount(itm.getUri(), vreq));
            body.put("roLink", getROLink(itm.getUri(), vreq));
    ...
    private static String RO_FULLTEXT_QUERY = ""
            + "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> \n"
            + "PREFIX uowvivo: <http://uowvivo.uow.edu.au/ontology/uowvivo#> \n"
            + "SELECT (str(?link) as ?roLink) WHERE { \n"
            + "    ?subject uowvivo:roFulltextUrl ?link \n"
            + "}" ;
    ...
    private static String getROLink(String subjectUri, VitroRequest vreq) {
        String queryStr = QueryUtils.subUriForQueryVar(RO_FULLTEXT_QUERY , "subject", subjectUri);
        log.debug("queryStr = " + queryStr);
        String roLink = null;
        try {
            //ResultSet results = QueryUtils.getQueryResults(queryStr, vreq);
            ...
            if (results.hasNext()) {
                QuerySolution soln = results.nextSolution();
                roLink = soln.getLiteral("roLink").toString();
            }
        } catch (Exception e) {
            log.error(e, e);
        }
