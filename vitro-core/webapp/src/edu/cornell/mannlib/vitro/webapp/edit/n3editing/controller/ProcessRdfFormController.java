...
public class ProcessRdfFormController extends FreemarkerHttpServlet{
	...
	protected ResponseValues processRequest(VitroRequest vreq) {			
	    ...
	    ProcessRdfForm prf = null;
        try {

            prf = new ProcessRdfForm(configuration, new NewURIMakerVitro(vreq.getWebappDaoFactory()));        
        ...
        //sneak the objectUri into return url if creating an object
        if(configuration.isCreateAndStayOnEditPage()){
	        if(prf != null && prf.getUrisForNewResources() != null){
	        	String appendToUrlPattern = "";
	        	for(String key: prf.getUrisForNewResources().keySet()){
	        		if(key.equals(configuration.getVarNameForObject())){
	        			try{
	        				appendToUrlPattern = "&objectUri="+URLEncoder.encode(prf.getUrisForNewResources().get(key), "UTF-8");
	        				break;
	        	...
	        	if(!configuration.getUrlPatternToReturnTo().contains("objectUri")){   
	        		configuration.setUrlPatternToReturnTo(configuration.getUrlPatternToReturnTo().concat(appendToUrlPattern));
	        	}
	        	log.info("after changes to urlpattern return to:"+configuration.getUrlPatternToReturnTo());        	
	        }
        }
