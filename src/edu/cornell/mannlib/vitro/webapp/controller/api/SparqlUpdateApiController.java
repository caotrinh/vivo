...
public class SparqlUpdateApiController extends VitroApiServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if(action == null || action.equals("sparqlUpdate")){
				UpdateRequest parsed = parseUpdateString(req);
				executeUpdate(req, parsed);
			} else if(action.equals("deleteIndividual")){
				executeDelIndividual(req);
			}
			do200response(resp);
	...
	private void executeDelIndividual(HttpServletRequest req){
		VitroRequest vreq = new VitroRequest(req);
		SearchIndexer indexer = ApplicationUtils.instance().getSearchIndexer();
		indexer.pause();
		String individualToDelete = req.getParameter("individualuri");
		try{
			IndividualDao dao = vreq.getWebappDaoFactory().getIndividualDao();
			dao.deleteIndividual(individualToDelete);
		} finally {
			indexer.unpause();
		}
	}
