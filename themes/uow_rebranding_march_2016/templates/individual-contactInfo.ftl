    ...
    <#if phone.statements?has_content> <#-- if there are any statements -->
        <ul id="individual-phone" role="list" <#if editable>style="list-style:none;margin-left:0;"</#if>>
            <#list phone.statements as statement>
                <li role="listitem">
                    <a class="phone" href="tel:${statement.number!}" title="${i18n().phone}">${statement.number!} <span class="hoverlight glyphicon glyphicon-phone contact-item phone"></span></a>
        ...
        <#if email.statements?has_content> <#-- if there are any statements -->
            <ul id="${listId}" class="individual-emails" role="list" <#if editable>style="list-style:none;margin-left:0;"</#if>>
                <#list email.statements as statement>
                    <li role="listitem">
                        <a class="email" href="mailto:${statement.emailAddress!}" title="${i18n().email}">${statement.emailAddress!} <span class="hoverlight glyphicon glyphicon-envelope contact-item"></span></a>
