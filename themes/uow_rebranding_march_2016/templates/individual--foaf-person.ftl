    ...
    <div class="col-md-6">
      <header>
        ...
        <#else>
          ...
          <#assign isHonorary = false>
          <#list individual.mostSpecificTypes as type>
            <#if type?contains("Honorary")>
                <h4>${type}</h4>
                <#assign isHonorary = true>
            </#if>
          </#list>
          <h3>
            <#--  Display preferredTitle if it exists; otherwise mostSpecificTypes -->
            <#assign role = propertyGroups.pullProperty("http://purl.obolibrary.org/obo/ARG_2000028","http://www.w3.org/2006/vcard/ns#Role")!>
            <#-- If positionTitle is unpopulated, display mostSpecificTypes -->
            <#if ! (role.statements)?has_content && ! isHonorary>
              <@p.mostSpecificTypesTitleDisplay individual />
            </#if>
            <#if role?has_content> <#-- true when the property is in the list, even if not populated (when editing) -->
              <#if (role.statements?size < 1) >
                <@p.addLinkWithLabel role editable />
              <#elseif editable>
                <h3>${role.name?capitalize!}</h3>
                <@p.verboseDisplay role />
              </#if>
              <#list role.statements as statement>
                <span class="display-title<#if editable>-editable</#if>">${statement.positionTitle}</span>
                <@p.editingLinks "${role.localName}" "${role.name}" statement editable role.rangeUri />
              </#list>
            </#if>
          </h3>
        </#if>
